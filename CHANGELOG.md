...
- Crash and on-screen error spam (`SFNozzleEffects.lua:419 attempt to compare boolean < number`) plus the follow-on freeze when Precision Farming is also installed (#636). Our See & Spray functions shared names with PF's ExtendedSprayerEffects spec but used a different signature, so the two specs overwrote each other. Renamed our functions to unique `sf`-prefixed names. (The MP liquid-refill and HUD desync parts of #636 are still being worked on.)
